...
declare function initializeHandleGizmos(viewport: any): void;
declare function getNewFileHandle(desc: string, mime: string, ext: string): Promise<any>;
declare function writeFile(handle: any, contents: string): Promise<void>;
declare function downloadFile(data: string, name: string, mime: string, ext: string): Promise<void>;
...
class Environment {
  ...
  time!: any; // THREE.Clock
  lastTimeRendered: number = 0.0;
...
class CascadeEnvironment {
  ...
  raycaster: any;
  highlightedObj: any = null;
  highlightedIndex: any;
  fogDist: number = 200;
  handles: any[] = [];
  gizmoMode: string = "translate";
  gizmoSpace: string = "local";
  loader: any;
  matcap: any;
  matcapMaterial: any;
  mainObject: any;
  ...
  constructor(goldenContainer: any) {
    ...
    this.matcap = this.loader.load('./textures/dullFrontLitMetal.png', (tex: any) => { this.environment.viewDirty = true; } );
    ...
    messageHandlers["combineAndRenderShapes"] = (payload: any) => {
      const [[facelist, edgelist], sceneOptions] = payload;
      (window as any).workerWorking = false;     // Untick this flag to allow Evaluations again
      ...
      let vertices: number[] = [], normals: number[] = [],  triangles: number[] = [], uvs: number[] = [], colors: number[] = []; let vInd = 0; let globalFaceIndex = 0;
      ...
      let lineVertices: any[] = []; let globalEdgeIndices: number[] = [];
      let curGlobalEdgeIndex = 0; let edgeVertices = 0;
      let globalEdgeMetadata: Record<any, any> = {}; globalEdgeMetadata[-1] = { start: -1, end: -1 };
      ...
      let lineColors: number[] = []; for ( let i = 0; i < lineVertices.length; i++ ) { lineColors.push( 0, 0, 0 ); }
    ...
    this.saveShapeSTEP = () => {
      ...
      messageHandlers["saveShapeSTEP"] = async (stepContent: string) => {
    ...
    this.mouse = { x: 0, y: 0 };
    this.goldenContainer.getElement().get(0).addEventListener('mousemove', (event: MouseEvent) => {
  ...
  animate(): void {}
  saveShapeSTEP(): void {}
  saveShapeSTL(): Promise<void> { return Promise.resolve(); }
  saveShapeOBJ(): Promise<void> { return Promise.resolve(); }
}
